...
def parse(arff_, is_path = True, dense_mode = True):
    ...
    try:
        ...
        obj = ArffStructure.from_arff(arff_parsed)
        return obj 
...
class ArffStructure:
    ...
    """
    def __init__(self):
        """ Use class methods to get an instance of this class.
        """
    ...
    @classmethod
    def from_arff(cls, arff_data):
        """ Reads and encapsulates arff_data which is a dictionary returned by the arff.py module.
        """
        attributes_list = arff_data['attributes']  # attribute list containing tupel (x,y). 
    ...
    @classmethod
    def from_labeledinstances(cls, X, classlabels):
            """ Takes labeled instances object X and parses it to a tflib.arffcontainer.ArffStructure. Look at the ArffStructure class constructor to see the meaning of the attributes that are expected to be assigned.

            X is a labeledinstances dictionary for example: {"instances":[[1.0,2.0],[3.0,4.0]],"labels":["A","B"]}
            classlabels is a list of strings for example: ["A","B"]
            """
            arffstruct = ArffStructure()
            # extract list for better access
            instances = X["instances"]
            labels = X["labels"]

            if len(instances) == 0:
                # no training data was given..
                raise ValueError(f"X={X} must have at least one entry.")# return doing nothing.
                
            arffstruct.__dict__[ "class_list"] = classlabels
            arffstruct.__dict__[ "entry_size"] = len(instances)
            arffstruct.__dict__[ "in_size"] = len(instances[0])
